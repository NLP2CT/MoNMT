...
def load_maskedlm_dataset(
    ...
    for k in itertools.count():
        ...
        # prependid = lang_dict.index("[{}]".format(lang)) if use_lgid else lang_dict.bos()
        # dataset = PrependTokenDataset(dataset, prependid)
...
def load_lm_datasets(
    ...
    for k in itertools.count():
        ...
        dataset = TokenBlockDataset(
            ...
            tokens_per_sample,
            pad=lang_dict.pad(),
            eos=lang_dict.index("[{}]".format(lang)) if use_lgid else lang_dict.eos(),
            break_mode=sample_break_mode,
...
class MultiTaskTranslationConfig(FairseqDataclass):
    ...
    ft_freeze: bool = field(
        default=False, metadata={"help": "prepend beginning of sentence token (<s>)"}
    )

    bt_freeze: bool = field(
        default=False, metadata={"help": "prepend beginning of sentence token (<s>)"}
    )

    srcdae_freeze_encoder: bool = field(
        default=False, metadata={"help": "prepend beginning of sentence token (<s>)"}
    )
    srcdae_freeze_decoder: bool = field(
        default=False, metadata={"help": "prepend beginning of sentence token (<s>)"}
    )
    
    tgtdae_freeze_encoder: bool = field(
        default=False, metadata={"help": "prepend beginning of sentence token (<s>)"}
    )
    tgtdae_freeze_decoder: bool = field(
        default=False, metadata={"help": "prepend beginning of sentence token (<s>)"}
    )
    ...
    ae: bool = field(
        default=False, metadata={"help": "user ae instead of srcdae or tgtdae"}
    )
...
@register_task("multitasklm", dataclass=MultiTaskTranslationConfig)
class MultitaskLM(FairseqTask):
    """
    ...
    def __init__(self, cfg: MultiTaskTranslationConfig, src_dict, tgt_dict, lm_targets=None):
        ...
        logger.info("[{}] final dictionary: {} types".format(cfg.source_lang, len(src_dict)))
        logger.info("[{}] final dictionary: {} types".format(cfg.target_lang, len(tgt_dict)))
        ...
        self.lambda_ft, self.lambda_ft_steps = parse_lambda_config(
            cfg.lambda_ft_config
        ) 
        self.lambda_bt, self.lambda_bt_steps = parse_lambda_config(
            cfg.lambda_bt_config
        ) 
        self.ft_freeze = cfg.ft_freeze
        self.bt_freeze = cfg.bt_freeze
        self.srcdae_freeze_encoder = cfg.srcdae_freeze_encoder
        self.tgtdae_freeze_encoder = cfg.tgtdae_freeze_encoder
        self.srcdae_freeze_decoder = cfg.srcdae_freeze_decoder
        self.tgtdae_freeze_decoder = cfg.tgtdae_freeze_decoder
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if (
            ...
            mt_dataset = load_langpair_dataset(
                ...
                append_source_id=self.cfg.append_source_id,
        ...
        bt_dataset = None
        if (
            self.lambda_bt > 0.0 or self.lambda_bt_steps is not None
        ) and split.startswith("train"):
            bt_path=self.cfg.btpath
            bt_dataset = load_langpair_dataset(
                bt_path,
                split,
                src,
                self.src_dict,
        ...
        ft_dataset = None
        if (
            self.lambda_ft > 0.0 or self.lambda_ft_steps is not None
        ) and split.startswith("train"):
            ft_path=os.path.join(data_path, self.cfg.monopath, "backtranslation_output", src+"2"+tgt)
            ft_dataset = load_langpair_dataset(
                ft_path,
                split,
                src,
                self.src_dict,
        ...
        if (
            ...
            mlm_dataset = load_maskedlm_dataset(
                ...
                use_lgid=self.cfg.append_source_id,
            )
        ...
        if (
            ...
            for k in itertools.count():
                ...
                filename = os.path.join(
                    data_path, self.cfg.monopath, "{}.{}-None.{}".format(split_k, src, src)
                )
                if split_k.startswith("valid"):
                    ...
                    if split_exists(split_k, src, tgt, src, data_path):
                        filename = os.path.join(
                            data_path, "{}.{}-{}.{}".format(split_k, src, tgt, src)
                        )
                    elif split_exists(split_k, tgt, src, src, data_path):
                        filename = os.path.join(
                            data_path, "{}.{}-{}.{}".format(split_k, tgt, src, src)
                        )
                    else:
                        raise FileNotFoundError(
                ...
                if not indexed_dataset.dataset_exists(filename, impl=self.cfg.dataset_impl):
                    break
                ...
                eos = None
                if self.cfg.append_source_id:
                    src_dataset1 = AppendTokenDataset(
                        src_dataset1, self.src_dict.index("[{}]".format(src))
                    )
                    if src_dataset2 is not None:
                        src_dataset2 = AppendTokenDataset(
                            src_dataset2, self.src_dict.index("[{}]".format(src))
                        )
                    eos = self.src_dict.index("[{}]".format(src))
                ...
                if not self.cfg.ae:
                    ...
                    noising_dataset = src_dataset1
        
                src_dae_datasets.append(LanguagePairDataset(
                    noising_dataset,
                    src_dataset1.sizes,
                    self.src_dict,
                    src_dataset2,
                ...
                logger.info(
                    "loaded {} blocks subset".format(
                        split_k,
                    )
                )
            dataset_lengths = np.array(
                [len(d) for d in src_dae_datasets],
                dtype=float,
            )
            logger.info(
                "loaded total {} blocks for all subset".format(
                    int(dataset_lengths.sum()),
                )
            )
            sample_probs = self._get_sample_prob(dataset_lengths)
            logger.info(
                "Sample probability by subset: {}".format(
                    {
                        id: "{0:.4f}".format(sample_probs[id])
                        for id, sp in enumerate(sample_probs)
                    }
                )
            )
            size_ratio = (sample_probs * dataset_lengths.sum()) / dataset_lengths
            logger.info(
                "Up/Down Sampling ratio by subset: {}".format(
                    {
                        id: "{0:.2f}".format(size_ratio[id])
                        for id, sr in enumerate(size_ratio)
                    }
                )
            )

            resampled_srcdaedatasets = [
                ResamplingDataset(
                    src_dae_datasets[i],
                ...
                for i, d in enumerate(src_dae_datasets)
            ]
            src_dae_dataset = ConcatDataset(
                resampled_srcdaedatasets,
            )
            logger.info(
                "src dae-{}: {} {} {} examples".format(
                    src,
                    data_path,
                    split,
                    len(src_dae_dataset),
        ...
        if (
            ...
            for k in itertools.count():
                ...
                filename = os.path.join(
                    data_path, self.cfg.monopath, "{}.{}-None.{}".format(split_k, tgt, tgt)
                )
                if split_k.startswith("valid"):
                    filename = os.path.join(
                        data_path, "{}.{}-{}.{}".format(split_k, src, tgt, tgt)
                    )
                    def split_exists(split, src, tgt, lang, data_path):
                        filename = os.path.join(data_path, "{}.{}-{}.{}".format(split, src, tgt, lang))
                        return indexed_dataset.dataset_exists(filename, impl=self.cfg.dataset_impl)
                    ...
                    if split_exists(split_k, src, tgt, tgt, data_path):
                        ...
                        filename = os.path.join(
                            data_path, "{}.{}-{}.{}".format(split_k, tgt, src, tgt)
                        )
                    else:
                        raise FileNotFoundError(
                            "Dataset not found: {} ({})".format(split_k, data_path)
                ...
                eos = None
                if self.cfg.append_source_id:
                    tgt_dataset1 = AppendTokenDataset(
                        tgt_dataset1, self.tgt_dict.index("[{}]".format(tgt))
                    )
                    if tgt_dataset2 is not None:
                        tgt_dataset2 = AppendTokenDataset(
                            tgt_dataset2, self.tgt_dict.index("[{}]".format(tgt))
                        )
                ...
                if not self.cfg.ae:
                    ...
                    noising_dataset = tgt_dataset1


        
                tgt_dae_datasets.append(LanguagePairDataset(
                    noising_dataset,
                    tgt_dataset1.sizes,
                    self.tgt_dict,
                    tgt_dataset2,
            ...
            dataset_lengths = np.array(
                [len(d) for d in tgt_dae_datasets],
                dtype=float,
            )
            logger.info(
                "loaded total {} blocks for all subset".format(
            ...
            resampled_tgtdaedatasets = [
                ...
                for i, d in enumerate(tgt_dae_datasets)
            ]
            tgt_dae_dataset = ConcatDataset(
                resampled_tgtdaedatasets,
            )
            logger.info(
                "tgt dae-{}: {} {} {} examples".format(
                    src,
                    data_path,
                    split,
                    len(tgt_dae_dataset),
        ...
        if ft_dataset is not None:
            datasetlist += [('ft', ft_dataset)]
        if bt_dataset is not None:
            datasetlist += [('bt', bt_dataset)]
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, constraints=None):
        return LanguagePairDataset(
            ...
            eos=self.tgt_lg_id if self.cfg.append_source_id else None
        )
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, ignore_grad=False
        ...
        if self.lambda_srcdae > 0.0:
            if self.srcdae_freeze_decoder:
                ...
                for p in model.decoder.parameters():
                    p.requires_grad = True
            elif self.srcdae_freeze_encoder:
        ...
        if self.lambda_tgtdae > 0.0:
            if self.tgtdae_freeze_encoder:
                ...
                for p in model.encoder.parameters():
                    p.requires_grad = True
            elif self.tgtdae_freeze_decoder:
                for p in model.decoder.parameters():
                    p.requires_grad = False
                if self.share_dictionary:
                    for p in model.encoder.embed_tokens.parameters():
        ...
        if self.lambda_ft > 0.0:
            if self.ft_freeze:
                ...
                if self.share_dictionary:
                    for p in model.encoder.embed_tokens.parameters():
                        p.requires_grad = True
                forward_backward(model, sample['ft'], 'ft', self.lambda_ft, 'ft')
                for p in model.decoder.parameters():
                    p.requires_grad = True
            else:
                forward_backward(model, sample['ft'], 'ft', self.lambda_ft, 'ft')

        if self.lambda_bt > 0.0:
            if self.bt_freeze:
                for p in model.encoder.parameters():
                    p.requires_grad = False
                if self.share_dictionary:
                    for p in model.encoder.embed_tokens.parameters():
                        p.requires_grad = True
                forward_backward(model, sample['bt'], 'bt', self.lambda_bt, 'bt')
                for p in model.encoder.parameters():
                    p.requires_grad = True
            else:
                forward_backward(model, sample['bt'], 'bt', self.lambda_bt, 'bt')
    ...
    def update_step(self, num_updates):
        ...
        if self.lambda_ft_steps is not None:
            self.lambda_ft = lambda_step_func(self.lambda_ft_steps, num_updates)
        if self.lambda_bt_steps is not None:
            self.lambda_bt = lambda_step_func(self.lambda_bt_steps, num_updates)

    def valid_step(self, sample, model, criterion):
        modes = ['mt', 'mlm', 'lm', 'srcdae', 'tgtdae', 'ft', 'bt'] if not self.only_eval_mt else ['mt']
        model.eval()
        with torch.no_grad():
            from collections import defaultdict
            ...
            for mode in modes:
                if(
                    ...
                    continue
                if mode in ['mt', 'srcdae', 'tgtdae', 'ft', 'bt']:
                    valid_model = model
                if mode == 'mlm':
