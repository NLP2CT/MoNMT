...
from .multilingual_reencoder import ReEncoder, Multilingual_Reencoder


logger = logging.getLogger(__name__)


@register_model("multilingual_mbart_with_reencoder")
class MultilingualBARTModelwithReencoder(BARTModel):

    def __init__(self, args, encoder, decoder):
        super().__init__(args, encoder, decoder)

        self.lang2fam=None
        if args.langauge_family is not None:
            self.lang2fam=dict()
            famindex=1
            for fam in args.langauge_family.strip().split(";"):
                fam_langs=fam.strip().split(',')
                for l in fam_langs:
                    self.lang2fam[l]=famindex
                famindex=famindex + 1
    ...
    def add_args(parser):
        ...
        parser.add_argument("--target-family-reencoder", default=False, action='store_true',
                            help='do not perform cross-attention')
        parser.add_argument("--source-family-reencoder", default=False, action='store_true',
                            help='do not perform cross-attention')
        parser.add_argument(
            "--language-family",
            default=None,
            type=str,
            metavar="PRETRAINED",
            help="language family split with ;, e.g. en,fr,de;zh,ja",
        )
    ...
    def load_state_dict(
        ...
        missing_keys, unexpected_keys = super().load_state_dict(state_dict, strict, model_cfg, args)
    ...
    def build_reencoder(cls, args):
        if self.lang2fam is None:
            reencoder = ReEncoder(args)
        else:
            reencoder = Multilingual_Reencoder(args, lang2fam=self.lang2fam)
    ...
    def forward(
        ...
        reencoder_out = self.reencoder(encoder_out["encoder_out"][0], encoder_out["encoder_padding_mask"][0], src_lang, tgt_lang)
...
@register_model_architecture("multilingual_mbart_with_reencoder", "multilingual_mbart_with_reencoder_large_for_ft")
...
# @register_model_architecture("multilingual_mbart_with_reencoder", "mbart_with_reencoder_transformer_large_for_ft")
