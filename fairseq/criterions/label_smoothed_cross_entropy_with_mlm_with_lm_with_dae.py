...
@dataclass
class LabelSmoothedCrossEntropyWithMLMWithLMCriterionConfig2(FairseqDataclass):
...
@register_criterion(
    "label_smoothed_cross_entropy_with_mlm_with_lm_with_dae", dataclass=LabelSmoothedCrossEntropyWithMLMWithLMCriterionConfig2
)
class LabelSmoothedCrossEntropyWithMLMWithLMWithDAECriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True, mode='mt'):
        ...
        """
        assert mode in ['mt', 'mlm', 'lm', 'dae']
        if mode == "mt" or mode == "dae":
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "loss", mt_loss+mlm_loss+lm_loss, sample_size, round=3
        )
        metrics.log_scalar(
            "mt_loss", mt_loss, mt_sample_size, round=3
        )
        metrics.log_scalar(
            "dae_loss", dae_loss, mt_sample_size, round=3
        )
        metrics.log_scalar(
