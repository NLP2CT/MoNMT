...
@dataclass
class LabelSmoothedCrossEntropyWithALLLMPRIORCriterionConfig(FairseqDataclass):
    ...
    prior_lambda: float = field(
        default=0.1,
        metadata={"help":"weight of the prior"}
    )
    prior_tau: float = field(
        default=1,
        metadata={"help":"temperature of the priorr"}
    )
    prior_smoothing: float = field(
        default=0.,
        metadata={"help":"epsilon for label smoothing, 0 means no label smoothing"}
    )
...
@register_criterion(
    "label_smoothed_cross_entropy_with_all_lmprior", dataclass=LabelSmoothedCrossEntropyWithALLLMPRIORCriterionConfig
)
class LabelSmoothedCrossEntropyWithALLLMPRIORCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True, mode='mt'):
        ...
        """
        assert mode in ['mt', 'mlm', 'lm', 'srcdae', 'tgtdae', 'ft', 'bt']
    ...
    def compute_priorlm_loss(self, model, net_output, prior_output, sample,
                     reduce=True):
    ...
    def forward_compute_mt_loss(self, model, sample, reduce=True):
        ...
        prior_loss, prior_nll_loss = self.compute_priorlm_loss(model,
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "loss", mt_loss+mlm_loss+lm_loss+srcdae_loss+tgtdae_loss+ft_loss+bt_loss, sample_size, round=3
        )
        metrics.log_scalar(
