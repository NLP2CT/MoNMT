...
def extract_soft_alignment_2(sample, model, src_punc_tokens, alignment_layer=None, tgt_punc_tokens=None, alignment_task=None):
    ...
    bsz, _ = src_sent.size()
    pad = model.decoder.dictionary.pad_index
    eos = model.decoder.dictionary.eos_index
    srclg=model.decoder.dictionary.index('[{}]'.format('en'))
    tgtlg=model.decoder.dictionary.index('[{}]'.format('de'))

    model.eval()
    alignment_heads = 1 if alignment_task=='usehead' else None
    encoder_out = model.encoder(src_sent, src_lengths=src_lengths)
    _, attn = model.decoder(prev_output_tokens, encoder_out=encoder_out, alignment_layer=alignment_layer, alignment_heads=alignment_heads)
    attn = attn['attn'][0]  ###  bsz x tgt_len x src_len
    ...
    for idx in range(bsz):
        ...
        tgt_idxs = (((tgt_sent[idx] != pad) & (tgt_sent[idx] != eos) & (tgt_sent[idx] != tgtlg)).nonzero(as_tuple=False).squeeze(dim=-1)) ## used to name the tgt sequence
        tgt_valid = ((tgt_sent[idx] != pad).nonzero(as_tuple=False).squeeze(dim=-1))  ## use to extract attn weights
        src_invalid = (((src_sent[idx] == pad) | (src_sent[idx] == eos) | (src_sent[idx] == srclg)).nonzero(as_tuple=False).squeeze(dim=-1))           
        src_token_to_word = get_token_to_word_mapping(src_sent[idx], [eos, pad, srclg])
        tgt_token_to_word = get_token_to_word_mapping(tgt_sent[idx], [pad, tgtlg])
...
def extract_soft_alignment_2_noshift(sample, model, src_punc_tokens, alignment_layer=None, tgt_punc_tokens=None, alignment_task=None):
    src_sent = sample['net_input']['src_tokens']
    src_lengths = sample['net_input']['src_lengths']
    prev_output_tokens =  sample['net_input']['prev_output_tokens']
    tgt_sent = sample['target'] 
    ...
    for idx in range(bsz):
        ...
        sample_id = sampleID[idx]
        alignments[sample_id] = []
        tgt_idxs = ((tgt_sent[idx] != pad) & (tgt_sent[idx] != eos) & (tgt_sent[idx] != tgtlg)).nonzero().squeeze(dim=-1) ## used to name the tgt sequence
        tgt_valid = ((tgt_sent[idx] != pad) & (tgt_sent[idx] != eos) & (tgt_sent[idx] != tgtlg)).nonzero().squeeze(dim=-1)  ## use to extract attn weights
        src_invalid = ((src_sent[idx] == pad) | (src_sent[idx] == eos) | (src_sent[idx] == srclg)).nonzero().squeeze(dim=-1)           
        src_token_to_word = get_token_to_word_mapping(src_sent[idx], [eos, pad, srclg])
        tgt_token_to_word = get_token_to_word_mapping(tgt_sent[idx], [eos, pad, tgtlg])
