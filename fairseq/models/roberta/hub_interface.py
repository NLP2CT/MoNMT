...
class RobertaHubInterface(nn.Module):
    ...
    def encode(
        ...
        if special_token != None:
            bpe_sentence = self.bpe.encode(sentence) + "[en]"
        else:
    ...
    def predict(self, head: str, tokens: torch.LongTensor, src_lengths=None, return_logits: bool = False):
        features = self.extract_features(tokens.to(device=self.device))
        logits = self.model.classification_heads[head](features, src_lengths)
        if return_logits:
            return logits
