...
class XTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.decoder_layerdrop = args.xdecoder_layerdrop
        self.share_input_output_embed = args.share_decoder_input_output_embed

        input_embed_dim = embed_tokens.embedding_dim
        embed_dim = args.xdecoder_embed_dim
        self.embed_dim = embed_dim
        self.output_embed_dim = args.xdecoder_output_dim
        ...
        if self.xdecoder_layerdrop > 0.0:
            self.layers = LayerDropModuleList(p=self.xdecoder_layerdrop)
        else:
            self.layers = nn.ModuleList([])
        self.layers.extend(
            [
                self.build_decoder_layer(args, no_encoder_attn)
                for _ in range(args.xdecoder_layers)
            ]
        )
        self.num_layers = len(self.layers)

        if args.xdecoder_normalize_before and not getattr(
    ...
    def forward(
        self,
        prev_output_tokens_decoder_out,
        ...
        x, extra = self.extract_features(
            ...
            concat_encoder_out=concat_encoder_out,
        )
    ...
    def extract_features_scriptable(
        ...
        # if positions is not None:
        #     x += positions
        x = prev_output_tokens_decoder_out
        ...
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        if concat_encoder_out and encoder_out is not None:
