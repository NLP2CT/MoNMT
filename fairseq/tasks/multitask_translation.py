...
@register_task("multitasktranslation", dataclass=MultiTaskTranslationConfig)
class MultitaskTranslationTask(FairseqTask):
    ...
    def valid_step(self, sample, model, criterion):
        modes = ['mt', 'mlm', 'lm'] if not self.only_eval_mt else ['mt']
        model.eval()
        with torch.no_grad():
            from collections import defaultdict
    ...
    def build_generator(
        ...
        if seq_gen_cls is None:
            if not getattr(args, "crossatt", False) and getattr(args, "num_lm_layers", False) and not getattr(args, "cross_self_attention", False):
