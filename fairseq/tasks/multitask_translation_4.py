...
@register_task("multitasktranslation4", dataclass=MultiTaskTranslationConfig)
class MultitaskTranslationTask4(FairseqTask):
    """
    ...
    def __init__(self, cfg: MultiTaskTranslationConfig, src_dict, tgt_dict, lm_targets=None):
        ...
        self.lambda_dae, self.lambda_dae_steps = parse_lambda_config(
            cfg.lambda_dae_config
        ) 
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if (
            self.lambda_dae > 0.0 or self.lambda_dae_steps is not None
        ) and (split.startswith("train") or split.startswith("valid")):
            ...
            dae_dataset = LanguagePairDataset(
                noising_dataset,
                tgt_dataset1.sizes,
                self.tgt_dict,
                tgt_dataset2,
        ...
        if dae_dataset is not None:
            datasetlist += [('dae', dae_dataset)]
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, ignore_grad=False
        ...
        if self.lambda_dae > 0.0:
            forward_backward(model, sample['dae'], 'dae', self.lambda_mt, 'dae')
    ...
    def update_step(self, num_updates):
        ...
        if self.lambda_dae_steps is not None:
            self.lambda_dae = lambda_step_func(self.lambda_dae_steps, num_updates)

    def valid_step(self, sample, model, criterion):
        modes = ['mt', 'mlm', 'lm', 'dae'] if not self.only_eval_mt else ['mt']
        model.eval()
        with torch.no_grad():
            from collections import defaultdict
