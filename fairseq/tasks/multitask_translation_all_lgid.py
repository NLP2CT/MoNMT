...
@register_task("multitasktranslationalllgid", dataclass=MultiTaskTranslationConfig)
class MultitaskTranslationTaskAllLGID(FairseqTask):
    """
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if (
            ...
            if split.startswith("valid"):
                ...
                if split_exists(split, src, tgt, src, data_path):
                    ...
                    filename = os.path.join(
                        data_path, "{}.{}-{}.{}".format(split, tgt, src, src)
                    )
                else:
                    raise FileNotFoundError(
                        "Dataset not found: {} ({})".format(split, data_path)
        ...
        if (
            ...
            if split.startswith("valid"):
                ...
                if split_exists(split, src, tgt, tgt, data_path):
                    filename = os.path.join(
                        data_path, "{}.{}-{}.{}".format(split, src, tgt, tgt)
                    )
                elif split_exists(split, tgt, src, tgt, data_path):
                    filename = os.path.join(
                        data_path, "{}.{}-{}.{}".format(split, tgt, src, tgt)
                    )
                else:
                    raise FileNotFoundError(
                        "Dataset not found: {} ({})".format(split, data_path)
